...
export async function fetchLotTasks(lotId: string): Promise<LotTask[]> {
  const { data, error } = await supabase
    .from("lot_tasks")
    .select("id,lot_id,title,description,order_index,status,due_date,completed_at")
    .eq("lot_id", lotId)
...
export async function updateLotTask(taskId: string, patch: Partial<CreateLotTaskInput> & { status?: LotTaskStatus }) {
  const payload: any = {};
